...
(** We use OCamls ints to store the bits. We index them from the
    ...
    bits that the don't store values. *)
let __msb = 1 lsl (__width - 1)
let __lsb = 1

(* Shift __lsb_masks to align with the remainder below. *)
let __lsb_masks =
  let a = Array.make (__width + 1) 0 in
  for i = 1 to __width do a.(i) <- a.(i-1) lor (1 lsl (i - 1)) done;
  a
...
let create ~size default =
  if size = 0 then { a = [| |] ; size }
    ...
    (*Printf.printf "r: %d, mask: %d\n" r __lsb_masks.(r); *)
    if default && r <> 0
    then arr.(n-1) <- __lsb_masks.(r);
    { a = arr; size }
  end
...
let __really_resize bv ~desired current =
  let a' = Array.make desired 0 in
  Array.blit bv.a 0 a' 0 current;
  bv.a <- a';
  bv.size <- __width * desired

let __resize bv desired =
  let current = Array.length bv.a in
  if desired > current 
  then __really_resize bv ~desired current

let resize bv desired ~size =
  __resize bv desired;
  bv.size <- size
...
let negate_self b =
  ...
  if r <> 0 then
    let n = Array.length b.a in
    b.a.(n - 1) <- __lsb_masks.(r) land b.a.(n - 1)
...
let negate b =
  ...
  if r <> 0 then begin
    let n = Array.length b.a in
    a.(n - 1) <- (__lsb_masks.(r) land a.(n - 1))
  end;
  { a ; size = b.size }
...
let union_into ~into bv =
  let n = Array.length bv.a in
  let c = Array.length into.a in
  if c < n
  then __really_resize into ~desired:n c;
  for i = 0 to n - 1 do into.a.(i) <- into.a.(i) lor bv.a.(i) done;
  into.size <- max into.size bv.size

(* To avoid potentially 2 passes, figure out what we need to copy. *)
let union b1 b2 =
  let n1 = Array.length b1.a in
  let n2 = Array.length b2.a in
  if n1 < n2
  then begin
    let into = copy b2 in
    for i = 0 to n1 - 1 do into.a.(i) <- into.a.(i) lor b1.a.(i) done;
    into
  end else begin
    let into = copy b1 in
    for i = 0 to n2 - 1 do into.a.(i) <- into.a.(i) lor b2.a.(i) done;
    into.size <- max into.size b2.size;    (* n1 = n2, but b2.size > b1.size *)
    into
  end
...
let inter_into ~into bv =
  ...
  let c = Array.length into.a in
  if c > n
  then __really_resize into ~desired:n n;
  for i = 0 to n - 1 do into.a.(i) <- into.a.(i) land bv.a.(i) done;
...
let inter b1 b2 =
  ...
  then begin
    let into = copy b1 in
    for i = 0 to n1 - 1 do into.a.(i) <- into.a.(i) land b2.a.(i) done;
    into
  end else begin
    let into = copy b2 in
    for i = 0 to n2 - 1 do into.a.(i) <- into.a.(i) land b1.a.(i) done;
    into.size <- min into.size b1.size;    (* n1 = n2, but b2.size > b1.size *)
    into
  end
...
(* Underlying size depends on the 'in_' set for diff. *)
let diff_into ~into bv =
  let n = min (Array.length into.a) (Array.length bv.a) in
  for i = 0 to n - 1 do into.a.(i) <- into.a.(i) land (lnot bv.a.(i)) done
